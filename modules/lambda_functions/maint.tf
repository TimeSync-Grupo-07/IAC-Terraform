...
resource "aws_lambda_function" "notification_team" {
  filename      = "${path.module}/code/dummy_lambda.zip"
...
resource "aws_s3_bucket_notification" "notification_trigger" {
  bucket = var.backup_bucket_name

  lambda_function {
    lambda_function_arn = aws_lambda_function.notification_team.arn
    events              = ["s3:ObjectCreated:*"]
    filter_prefix       = ""
    filter_suffix       = ".csv"
  }

  depends_on = [ aws_lambda_permission.allow_s3_to_invoke_notification ]
}

resource "aws_lambda_permission" "allow_lambda_notification_to_publish_sns" {
  statement_id  = "AllowRawLambdaToPublishSNS"
  action        = "lambda:sucess"
  principal     = "lambda.amazonaws.com"
  function_name = aws_lambda_function.notification_team.function_name
  source_arn    = aws_lambda_function.notification_team.arn
}
...
resource "aws_lambda_permission" "allow_s3_to_invoke_notification" {
